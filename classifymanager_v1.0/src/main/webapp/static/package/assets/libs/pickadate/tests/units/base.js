...
test("Picker alternate API", function () {
  ...
  strictEqual(
    $input.pickadate("get", "start"),
    picker.get("start"),
    "Methods are passed forward"
  );
  strictEqual(
    $input.pickadate("component"),
    picker.component,
    "Objects are passed forward"
  );
});
...
test("Default hidden prefix & suffix", function () {
  var picker = this.picker;
  strictEqual(
    picker.$node[0].name + "_submit",
    picker._hidden.name,
    "Correct hidden element `name`"
  );
});
...
test("Custom hidden prefix & suffix", function () {
  var picker = this.picker;
  strictEqual(
    "prefixed__" + picker.$node[0].name + "__suffixed",
    picker._hidden.name,
    "Correct hidden element `name`"
  );
});
...
test("Open with alternate focus", function () {
  ...
  setTimeout(function () {
    ok(
      !picker.get("open") &&
        picker.$node[0].className === klasses.input + " " + klasses.active &&
        picker.$root[0].className === klasses.picker + " " + klasses.opened &&
        document.activeElement !== picker.$node[0],
...
test("Set and clear", function () {
  ...
  strictEqual(
    ...
    "Set value as default selection after “enter”"
  );
