...
!(function ($) {
  ...
  var rowAttr = function (row, index) {
    ...
    if (!row.IsParent) {
      obj[dataTTParentId.toString()] =
        parentId === undefined ? index : parentId;
    } else {
      parentId = index;
  ...
  var getNewRow = function (that, result, index) {
    var newRow = {};
    for (var i = 0; i < that.options.groupByField.length; i++) {
      newRow[that.options.groupByField[i].toString()] =
        result[index][0][that.options.groupByField[i]];
    }
          ...
          .on("post-body.bs.table", function () {
            ...
            if (that.options.groupByInitExpanded !== undefined) {
              if (typeof that.options.groupByInitExpanded === "number") {
                ...
                that.options.groupByInitExpanded.toLowerCase() === "all"
              ) {
                that.expandAll();
              }
            }
    ...
    if (!this.options.sortName) {
      if (this.options.groupBy && this.options.groupByField.length > 0) {
        this.options.groupByField =
          typeof this.options.groupByField === "string"
            ? this.options.groupByField
                .replace("[", "")
                .replace("]", "")
                .replace(/ /g, "")
  ...
  BootstrapTable.prototype.refreshGroupByField = function (groupByFields) {
    if (
      !$.fn.bootstrapTable.utils.compareObjects(
        this.options.groupByField,
        groupByFields
      )
    ) {
