...
(function () {
  ...
  test(
    "clear - after clear with force option toast with focus disappears",
    1,
    function () {
      //Arrange
      var $toast;
      ...
      ok(
        ...
        "Focused toast after a clear with force is not visible"
      );
      //Teardown
      resetContainer();
    }
  ...
  asyncTest(
    "clear and show - show 2 toasts, clear both, then show 1 more",
    2,
    function () {
      //Arrange
      var $toast = [];
      ...
      setTimeout(function () {
        ...
        //Assert
        equal(
          $toast[2].find("div.toast-title").html(),
          sampleTitle + "-3-Visible",
          "Finds toast after a clear"
        );
  ...
  asyncTest(
    "clear and show - after clear new toast creates container",
    1,
    function () {
      //Arrange
      var $toast = [];
      ...
      setTimeout(function () {
        ...
        //Assert
        equal(
          $(selectors.container).find("div.toast-title").html(),
          sampleTitle + "-3-Visible",
          "Finds toast after a clear"
        ); //Teardown
        resetContainer();
  ...
  asyncTest("clear and show - clear toast after hover", 1, function () {
    ...
    setTimeout(function () {
      //Assert
      ok(
        $container.find("div.toast-title").length === 0,
        "Toast clears after a mouse hover"
      ); //Teardown
      resetContainer();
      start();
    }, 500);
  ...
  asyncTest("clear and show - do not clear toast after hover", 1, function () {
    ...
    setTimeout(function () {
      //Assert
      ok(
        $container.find("div.toast-title").length === 1,
        "Toast does not clear after a mouse hover"
      ); //Teardown
      resetContainer();
      start();
    }, 500);
  ...
  test(
    "clear and show - after clear all toasts new toast still appears",
    1,
    function () {
      //Arrange
      var $toast = [];
  ...
  test("info - escape html", 2, function () {
    ...
    //Assert
    equal(
      $toast.find("div.toast-title").html(),
      "html &lt;u&gt;title&lt;/u&gt;",
      "Title is escaped"
    );
    equal(
      $toast.find("div.toast-message").html(),
      "html &lt;strong&gt;message&lt;/strong&gt;",
      "Message is escaped"
    );
    //Teardown
  ...
  test("close button disabled", 1, function () {
    ...
    //Assert
    equal(
      $toast.find("button.toast-close-button").length,
      0,
  ...
  test("close button has type=button", 1, function () {
    ...
    //Assert
    equal(
      $toast.find('button[type="button"].toast-close-button').length,
      1,
  ...
  asyncTest("close button duration", 1, function () {
    ...
    setTimeout(function () {
      ...
      ok(
        $container && $container.children().length === 0,
        "close button should support own hide animation"
      );
      //Teardown
  ...
  test("progress bar disabled", 1, function () {
    ...
    //Assert
    equal(
      $toast.find("div.toast-progress").length,
      0,
      "progress bar should not exist with progressBar=false"
    );
    //Teardown
    $toast.remove();
    clearContainerChildren();
  ...
  test("progress bar enabled", 1, function () {
    ...
    equal(
      $toast.find("div.toast-progress").length,
      1,
      "progress bar should exist with progressBar=true"
    );
    //Teardown
    $toast.remove();
    clearContainerChildren();
  ...
  test("ltr toastr does not have .rtl class", 1, function () {
    ...
    //Assert
    ok(
      $toast.hasClass("rtl") === false,
      "ltr div container does not have .rtl class"
    );
    //Teardown
    $toast.remove();
    clearContainerChildren();
  ...
  test("toastr success has aria polite", 1, function () {
    ...
    ok(
      ...
      "success toast has aria-live of polite"
    );
  ...
  test("toastr info has aria polite", 1, function () {
    ...
    ok(
      $toast.attr("aria-live") === "polite",
      "info toast has aria-live of polite"
    );
  ...
  test("toastr warning has aria assertive", 1, function () {
    ...
    ok(
      ...
      "warning toast has aria-live of assertive"
    );
  ...
  test("toastr error has aria assertive", 1, function () {
    ...
    ok(
      $toast.attr("aria-live") === "assertive",
      "error toast has aria-live of assertive"
    );
  ...
  test(
    "event - message appears when no show or hide method functions provided",
    1,
    function () {
      //Arrange
      //Act
  ...
  test(
    "event - prevent duplicate sequential toasts, but allow previous after clear.",
    1,
    function () {
      toastr.options.preventDuplicates = true;
  ...
  test(
    "event - allow preventDuplicates option to be overridden.",
    1,
    function () {
      var $toast = [];
  ...
  module("subscription");
  asyncTest(
    "subscribe - triggers 2 visible and 2 hidden response notifications while clicking on a toast",
    1,
    function () {
      //Arrange
      var $toast = [];
  ...
  test("Newest toast on top", 1, function () {
    ...
    ok(
      containerHtml.indexOf("First toast") >
        containerHtml.indexOf("Second toast"),
      "Newest toast is on top"
    );
    //Teardown
    $first.remove();
    $second.remove();
  ...
  test("Container - position bottom-right", 1, function () {
    ...
    //Assert
    ok(
      $container.hasClass(positionClasses.bottomRight),
  ...
  test("Container - position bottom-left", 1, function () {
    ...
    //Assert
    ok(
      $container.hasClass(positionClasses.bottomLeft),
      "Has position bottom left"
    );
    //Teardown
    $toast.remove();
    resetContainer();
  ...
  test("Container - position top-center", 1, function () {
    ...
    //Assert
    ok(
      $container.hasClass(positionClasses.topCenter),
      "Has position top center"
    );
    //Teardown
    $toast.remove();
    resetContainer();
  ...
  test("Container - position bottom-center", 1, function () {
    ...
    //Assert
    ok(
      $container.hasClass(positionClasses.bottomCenter),
      "Has position bottom center"
    );
    //Teardown
    $toast.remove();
    resetContainer();
