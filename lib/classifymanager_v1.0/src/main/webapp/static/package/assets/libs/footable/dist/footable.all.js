...
(function ($, w, undefined) {
  w.footable = {
    options: {
      ...
      columnDataSelector:
        "> thead > tr:last-child > th, > thead > tr:last-child > td", //the selector used to find the column data in the thead
    ...
    plugins: {
      _validate: function (plugin) {
        ...
        if (w.footable.options.debug === true)
          console.log(
            'Validation succeeded for plugin "' + p["name"] + '".',
            p
          );
        return true;
      },
  ...
  $.fn.footable = function (options) {
    ...
    if (options.breakpoints) {
      // clear the default breakpoints if the user specifies one
  ...
  function Footable(t, o, id) {
    ...
    ft.bindToggleSelectors = function () {
      ...
      $table
        .find(opt.toggleSelector)
        .unbind(trg.toggleRow)
        .bind(trg.toggleRow, function (e) {
    ...
    ft.getColumnData = function (th) {
      ...
      data.hide["default"] =
        $th.data("hide") === "all" || $.inArray("default", hide) >= 0;

      var hasBreakpoint = false;
      for (var name in opt.breakpoints) {
        data.hide[name] =
          $th.data("hide") === "all" || $.inArray(name, hide) >= 0;
    ...
    ft.redraw = function () {
      ...
      $table
        .find("> tbody > tr." + cls.detailShow + ":visible")
        .each(function () {
          var $next = $(this).next();
...
(function ($, w, undefined) {
  ...
  function initPagination($table, cols, options) {
    if (options) {
      ...
      if ($tfoot.size() === 0) {
        $tfoot = $('<tfoot class="hide-if-no-paging"></tfoot>').appendTo(
          $table
        );
      }
  ...
  function setToggleColumn(cols) {
    ...
    for (var i = 0, len = cols.length; i < len; i++) {
      ...
      if (
        col.data &&
        (col.data.toggle === true || col.data.toggle === "true")
      ) {
        return;
      }
    }
  ...
  function Grid() {
    ...
    grid.init = function (ft) {
      ...
      $table
        ...
        .on(
          {
            "click.grid": function (event) {
              ...
              if (!options.multiSelect && !$tr.hasClass(options.activeClass)) {
                $table
                  .find("tbody tr." + options.activeClass)
        ...
        .on("click.grid", "thead input:checkbox.checkAll", function (event) {
    ...
    grid.getSelected = function () {
      var options = grid.footable.options.grid,
        $selected = $(grid.footable.table).find(
          "tbody>tr." + options.activeClass
        );
  ...
  function Filter() {
          ...
          .bind({
            "footable_initialized.filtering": function (e) {
              ...
              var data = {
                ...
                timeout:
                  $table.data("filter-timeout") || ft.options.filter.timeout,
                minimum:
                  $table.data("filter-minimum") || ft.options.filter.minimum,
                disableEnter:
                  $table.data("filter-disable-enter") ||
                  ft.options.filter.disableEnter,
              };
    ...
    p.showRow = function (row, ft) {
      var $row = $(row),
        ...
        $table = $(ft.table);
      if (
        $table.hasClass("breakpoint") &&
        $row.hasClass("footable-detail-show") &&
        $next.hasClass("footable-row-detail")
      ) {
        $row.add($next).show();
...
(function ($, w, undefined) {
  ...
  function pageInfo(ft) {
    ...
    this.limitNavigation = parseInt(
      data.limitNavigation ||
        ft.options.limitNavigation ||
        defaults.limitNavigation,
      10
    );
    this.limitPreviousText =
      data.limitPreviousText || ft.options.limitPreviousText;
  ...
  function Paginate() {
    ...
    p.createNavigation = function (ft, tbody) {
      ...
      if ($nav.length === 0) {
        ...
        if (
          $nav.parents("table:first").length > 0 &&
          $nav.parents("table:first") !== $(ft.table)
        )
          return;
    ...
    p.paginate = function (ft, newPage) {
      ...
      if (info.currentPage !== newPage) {
        ...
        var event = ft.raise("footable_paging", {
          page: newPage,
          size: info.pageSize,
        });
        if (event && event.result === false) return;
    ...
    p.setPagingClasses = function (nav, currentPage, pageCount) {
      nav
        .find("li.footable-page > a[data-page=" + currentPage + "]")
        .parent()
        .addClass("active");
  ...
  function Sort() {
          ...
          .bind({
            "footable_initialized.sorting": function (e) {
              ...
              $table
                ...
                .each(function (ec) {
                  var $th = $(this),
                    column = ft.columns[$th.index()];
                  if (
                    column.sort.ignore !== true &&
                    !$th.hasClass(cls.sortable)
                  ) {
              ...
              $table
                ...
                .unbind("click.footable")
                .bind("click.footable", function (ec) {
                  ec.preventDefault();
                  $th = $(this);
    ...
    p.doSort = function (columnIndex, ascending) {
      ...
      var $table = $(ft.table),
        ...
        $th = $table.find(
          "> thead > tr:last-child > th:eq(" + columnIndex + ")"
        ),
        cls = ft.options.classes.sort,
