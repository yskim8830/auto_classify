QUnit.module("repeater", {
  setup: function () {
    ...
    this.$secondRepeaterAddButton = this.$secondRepeater.find(
...
QUnit.test("instantiate with no first item", function (assert) {
  ...
  assert.strictEqual(
    this.$repeater.find("[data-repeater-item]").css("display"),
    "none",
...
QUnit.test(
  "instantiate group of repeaters with a single repeater() call",
  function (assert) {
...
QUnit.asyncTest("custom hide callback", function (assert) {
  ...
  $repeater.repeater({
    hide: function (removeItem) {
      ...
      assert.strictEqual($(this).is(":hidden"), false, "element is not hidden");
...
QUnit.test("isFirstItemUndeletable configuration option", function (assert) {
  ...
  var $firstDeleteButton = this.$repeater
    .find("[data-repeater-item]")
    .first()
    .find("[data-repeater-delete]");

  assert.strictEqual(
    $firstDeleteButton.length,
    0,
    "first delete button is removed"
  );
});
