...
!(function ($) {
  ...
  var getTableObjectFromCurrentTarget = function (currentTarget) {
    ...
    return currentTarget.is("table")
      ? currentTarget
      : currentTarget.parents().find(".table");
  };
    ...
    if (window.event.button === 0) {
      ...
      if (window.event.shiftKey) {
        selectRowsBetweenIndexes(
          [
            that.bootstrapTable("getOptions").multipleSelectRowLastSelectedRow
              .rowIndex,
  ...
  var toggleRow = function (row, that, clearAll, useShift) {
    if (clearAll) {
      ...
      that.bootstrapTable("getOptions").multipleSelectRowLastSelectedRow =
        undefined;
      row.removeClass(
        that.bootstrapTable("getOptions").multipleSelectRowCssClass
      );
  ...
  var selectRowsBetweenIndexes = function (indexes, that) {
    ...
    for (var i = indexes[0]; i <= indexes[1]; i++) {
      toggleRow(
        that.bootstrapTable("getOptions").multipleSelectRowRows[i - 1],
        that,
        false,
        true
      );
  ...
  var clearAll = function (that) {
    for (
      var i = 0;
      i < that.bootstrapTable("getOptions").multipleSelectRowRows.length;
      i++
    ) {
      toggleRow(
        that.bootstrapTable("getOptions").multipleSelectRowRows[i],
        that,
        true,
        false
      );
    ...
    if (this.options.multipleSelectRow) {
      ...
      this.$el.on("post-body.bs.table", function (e) {
        setTimeout(function () {
          ...
          that.options.multipleSelectRowRows
