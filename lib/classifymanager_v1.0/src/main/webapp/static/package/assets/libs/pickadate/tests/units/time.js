...
test("Clock stage", function () {
  strictEqual(
    this.picker.$root.find("[data-pick]").length,
    48,
    "48 selectable times"
  );
});
...
test("Properties", function () {
  ...
  strictEqual(
    picker.get("now").pick,
    (interval + nowMinutes - (nowMinutes % interval)) % 1440,
    "Default “now” is: " + picker.get("now", "HH:i")
  );
...
test("`interval`", function () {
  ...
  strictEqual(
    $root.find("." + $.fn.pickatime.defaults.klass.listItem).length,
    12,
    "12 selectable times"
  );
  ...
  strictEqual(
    picker.get("interval"),
    120,
    "Interval unaffected by non-integer"
  );
});
...
test("`select`", function () {
  ...
  strictEqual(
    ...
    180,
    "`select` using a number: " + picker.get("select", "HH:i")
  );
  ...
  strictEqual(
    picker.get("select").pick,
    270,
    "`select` using a JS date object: " + picker.get("select", "HH:i")
  );
  ...
  strictEqual(
    picker.get("select").pick,
    540,
    "`select` using an array: " + picker.get("select", "HH:i")
  );
...
test("`highlight`", function () {
  ...
  strictEqual(
    picker.get("highlight").pick,
    180,
    "`highlight` using a number: " + picker.get("highlight", "HH:i")
  );
  ...
  strictEqual(
    picker.get("highlight").pick,
    270,
    "`highlight` using a JS date object: " + picker.get("highlight", "HH:i")
  );
  deepEqual(picker.get("view"), picker.get("highlight"), "`view` updated");
  strictEqual(picker.get("select"), null, "`select` unaffected");
  strictEqual(picker.get("min").pick, 0, "`min` unaffected");
  ...
  strictEqual(
    picker.get("highlight").pick,
    540,
    "`highlight` using an array: " + picker.get("highlight", "HH:i")
  );
  deepEqual(picker.get("view"), picker.get("highlight"), "`view` updated");
  strictEqual(picker.get("select"), null, "`select` unaffected");
  strictEqual(picker.get("min").pick, 0, "`min` unaffected");
...
test("`view`", function () {
  ...
  strictEqual(
    ...
    180,
    "`view` using a number: " + picker.get("view", "HH:i")
  );
  strictEqual(
    picker.get("highlight").pick,
    picker.get("now").pick,
  ...
  strictEqual(
    picker.get("view").pick,
    270,
    "`view` using a JS date object: " + picker.get("view", "HH:i")
  );
  strictEqual(
    picker.get("highlight").pick,
    picker.get("now").pick,
  ...
  strictEqual(
    picker.get("view").pick,
    540,
    "`view` using an array: " + picker.get("view", "HH:i")
  );
  strictEqual(
    picker.get("highlight").pick,
    picker.get("now").pick,
...
test("`min` using integers", function () {
  ...
  picker.set("min", -3);
  strictEqual(
    (picker.get("min").pick + interval * 3) % 1440,
    picker.get("now").pick,
    "`min` using a negative number: " + picker.get("min", "HH:i")
  );
  ...
  strictEqual(
    picker.get("min").pick,
    (picker.get("now").pick + interval * 3) % 1440,
    "`min` using a positive number: " + picker.get("min", "HH:i")
  );
...
test("`min` using booleans", function () {
  ...
  deepEqual(
    picker.get("min"),
    picker.get("now"),
    "`min` using `true`: " + picker.get("min", "HH:i")
  );
  deepEqual(picker.get("select"), null, "`select` unaffected");
  ...
  strictEqual(
    picker.get("min").time,
    0,
    "`min` using `false`: " + picker.get("min", "HH:i")
  );
  deepEqual(picker.get("select"), null, "`select` unaffected");
  deepEqual(
    picker.get("highlight"),
...
test("`min` using arrays", function () {
  ...
  strictEqual(
    picker.get("min").pick,
    120,
    "`min` using an array: " + picker.get("min", "HH:i")
  );
  deepEqual(picker.get("select"), null, "`select` unaffected");
  if (picker.get("min").pick > picker.get("now").pick) {
    deepEqual(
...
test("`min` using JS dates", function () {
  ...
  strictEqual(
    picker.get("min").pick,
    270,
    "`min` using a JS date: " + picker.get("min", "HH:i")
  );
  deepEqual(picker.get("select"), null, "`select` unaffected");
  if (picker.get("min").pick > picker.get("now").pick) {
    deepEqual(
...
test("`max` using integers", function () {
  ...
  strictEqual(
    picker.get("max").pick,
    (picker.get("now").pick + interval * 3) % 1440,
    "`max` using a positive number: " + picker.get("max", "HH:i")
  );
  ...
  strictEqual(
    picker.get("max").pick,
    maxPickTime,
    "`max` using a negative number: " + picker.get("max", "HH:i")
  );
...
test("`max` using booleans", function () {
  ...
  deepEqual(
    picker.get("max"),
    picker.get("now"),
    "`max` using `true`: " + picker.get("max", "HH:i")
  );
  strictEqual(picker.get("select"), null, "`select` unaffected");
  strictEqual(
    picker.get("highlight").pick,
  ...
  deepEqual(
    picker.get("max").pick,
    1410,
    "`max` using `false`: " + picker.get("max", "HH:i")
  );
  strictEqual(picker.get("select"), null, "`select` unaffected");
  strictEqual(
    picker.get("highlight").pick,
...
test("`max` using arrays", function () {
  ...
  strictEqual(
    picker.get("max").pick,
    870,
    "`max` using an array: " + picker.get("max", "HH:i")
  );
  strictEqual(picker.get("select"), null, "`select` unaffected");
  if (picker.get("max").pick < picker.get("now").pick) {
    deepEqual(
...
test("`max` using JS dates", function () {
  ...
  strictEqual(
    picker.get("max").pick,
    960,
    "`max` using a JS date: " + picker.get("max", "HH:i")
  );
  deepEqual(picker.get("select"), null, "`select` unaffected");
  if (picker.get("max").pick < picker.get("now").pick) {
    deepEqual(
...
test("`disable` and `enable` using integers", function () {
  ...
  $root.find("[data-pick]").each(function () {
    ...
    if (disableCollection.indexOf(hour) > -1) {
      ok(
        $this.hasClass($.fn.pickatime.defaults.klass.disabled),
        "Disabled time: " + $this.html()
      );
    } else {
      ok(
        !$this.hasClass($.fn.pickatime.defaults.klass.disabled),
        "Enabled time: " + $this.html()
      );
    }
  });
...
test("`disable` and `enable` using arrays", function () {
  ...
  $root
    .find("." + $.fn.pickatime.defaults.klass.listItem)
    .each(function (index, item) {
      if (index === 2 || index === 9 || index === 36 || index === 47) {
        ok(
          $(item).hasClass($.fn.pickatime.defaults.klass.disabled),
          "Time is disabled: " + item.innerHTML
        );
      } else {
        ok(
          !$(item).hasClass($.fn.pickatime.defaults.klass.disabled),
          "Time is enabled: " + item.innerHTML
        );
      }
    });
  ...
  deepEqual(
    picker.get("disable"),
    disableCollection,
    "Disabled collection with specified time inverted"
  );
});
...
test("`disable` and `enable` using JS times", function () {
  ...
  deepEqual(
    picker.get("disable"),
    [new Date(2014, 2, 2, 1), new Date(2014, 2, 2, 3)],
    "Disabled time removed"
  );
...
test("`disable` and `enable` using booleans", function () {
  ...
  deepEqual(
    picker.get("disable"),
    disableCollection,
    "Disabled times added to collection"
  );

  picker.set("disable", false);
  deepEqual(picker.get("disable"), [], "Disabled collection reset");
  strictEqual(
    $root.find("." + $.fn.pickatime.defaults.klass.disabled).length,
    0,
    "No times disabled"
  );
...
test("`disable` and `enable` using ranges", function () {
  ...
  $root.find("[data-pick]").each(function (indexCell, tableCell) {
    if (indexCell >= 4 && indexCell <= 29) {
      ok(
        $(tableCell).hasClass($.fn.pickatime.defaults.klass.disabled),
        "Time is disabled: " + tableCell.innerHTML
      );
    } else {
      ok(
        !$(tableCell).hasClass($.fn.pickatime.defaults.klass.disabled),
        "Time is enabled: " + tableCell.innerHTML
      );
    }
  });
  ...
  disableCollection = [
    { from: new Date(2014, 2, 7, 5, 30), to: new Date(2014, 2, 7, 19) },
  ];
  picker.set("disable", disableCollection);
...
test("`disable` and `enable` using relative ranges", function () {
  ...
  deepEqual(
    picker.get("disable"),
    disableCollection,
    "Disabled range relative to now with date object"
  );
  ...
  deepEqual(
    picker.get("disable"),
    disableCollection,
    "Disabled range relative to now with array"
  );
  ...
  deepEqual(
    picker.get("disable"),
    disableCollection,
    "Disabled range relative to now with positive integer"
  );
  ...
  deepEqual(
    picker.get("disable"),
    disableCollection,
    "Disabled range relative to now with negative integer"
  );
});
...
test("`disable` and `enable` using overlapping ranges", function () {
  ...
  picker.set("disable", [{ from: [2, 0], to: [10, 30] }]);
  picker.set("enable", [{ from: [5, 30], to: [7, 30] }]);
  ...
  $root.find("[data-pick]").each(function (indexCell, tableCell) {
    if (
      (indexCell >= 4 && indexCell < 11) ||
      (indexCell > 15 && indexCell <= 21)
    ) {
      ok(
        $(tableCell).hasClass($.fn.pickatime.defaults.klass.disabled),
        "Time is disabled: " + tableCell.innerHTML
  ...
  picker.set("disable", false);
  picker.set("disable", [{ from: [2, 0], to: [10, 30] }]);
  picker.set("enable", [{ from: [0, 30], to: [7, 30] }]);
  ...
  picker.set("disable", false);
  picker.set("disable", [{ from: [2, 0], to: [10, 30] }]);
  picker.set("enable", [{ from: [7, 30], to: [14, 0] }]);
...
test("`select`", function () {
  ...
  deepEqual(
    [selected.hour, selected.mins],
    [1, 30],
    "Scoped `select` into interval range"
  );
...
test("`highlight`", function () {
  ...
  deepEqual(
    [highlighted.hour, highlighted.mins],
    [1, 30],
    "Scoped `highlight` into interval range"
  );
...
test("`view`", function () {
  ...
  deepEqual(
    [viewset.hour, viewset.mins],
    [1, 30],
    "Scoped `view` into interval range"
  );
...
test("Select", function () {
  ...
  for (var i = 0; i < 48; i += 1) {
    ...
    strictEqual(
      picker.get("select").pick,
      i * interval,
      "Selected " + picker.get("select", "h:i A")
    );
    strictEqual(
      picker.get("value"),
      picker.get("select", "h:i A"),
...
test("Clear", function () {
  ...
  strictEqual(
    picker.get("value"),
    picker.get("select", $.fn.pickatime.defaults.format),
    "Value updated"
  );
...
test("Select", function () {
  ...
  for (var i = 0; i < 48; i += 1) {
    ...
    deepEqual(
      picker.get("select"),
      picker.get("highlight"),
      "Select updated to: " + picker.get("select", "HH:i")
    );
  }
});
...
test("Highlight", function () {
  ...
  for (var i = 1; i < 48; i += 1) {
    ...
    strictEqual(
      picker.get("highlight").pick,
      30 * i,
      "Key “down” to `highlight`: " + picker.get("highlight", "h:i A")
    );
  ...
  for (var j = 2; j < 49; j += 1) {
    ...
    strictEqual(
      picker.get("highlight").pick,
      1440 - 30 * j,
      "Key “up” to `highlight`: " + picker.get("highlight", "h:i A")
    );
    deepEqual(picker.get("view"), picker.get("highlight"), "Updated `view`");
  }
  ...
  for (var k = 1; k < 48; k += 1) {
    ...
    strictEqual(
      picker.get("highlight").pick,
      30 * k,
      "Key “right” to `highlight`: " + picker.get("highlight", "h:i A")
    );
    deepEqual(picker.get("view"), picker.get("highlight"), "Updated `view`");
  }
