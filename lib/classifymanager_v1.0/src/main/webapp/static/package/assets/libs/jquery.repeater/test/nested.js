QUnit.module("nested-repeater", {
  setup: function () {
    ...
    this.$outerAddButton = this.$fixture.find(
      ".outer-repeater > [data-repeater-create]"
    );
    this.$innerAddButton = this.$fixture.find(
...
QUnit.test("isFirstItemUndeletable configuration option", function (assert) {
  ...
  var $outerItems = this.$outerRepeater.find(
    '[data-repeater-list="outer-group"] > [data-repeater-item]'
  );
  ...
  assert.strictEqual(
    this.$outerRepeater
      .find("[data-repeater-item].outer")
      .first()
      .find("[data-repeater-delete].outer").length,
    0,
  ...
  assert.strictEqual(
    this.$innerRepeater
      ...
      .first()
      .find("[data-repeater-delete]").length,
    0,
...
QUnit.test("setList", function (assert) {
  ...
  var $items = this.$outerRepeater.find(
    '[data-repeater-list="outer-group"] > [data-repeater-item]'
  );
...
QUnit.test("add item nested outer", function (assert) {
  ...
  assert.strictEqual(
    $items
      .first()
      .find('[data-repeater-list="inner-group"] > [data-repeater-item]').length,
    1,
    "does not duplicate first inner repeater"
  );
  ...
  assert.strictEqual(
    $items
      ...
      .find('[data-repeater-list="inner-group"] > [data-repeater-item]').length,
    1,
    "does not duplicate last inner repeater"
  );
...
QUnit.test("delete added item outer from repeated outer", function (assert) {
  ...
  assert.deepEqual(getNamedInputValues(this.$outerRepeater), {
    "outer-group[0][text-input]": "A",
    "outer-group[0][inner-group][0][inner-text-input]": "B",
  });
});
...
QUnit.test("add item nested inner from repeated outer", function (assert) {
  ...
  var $lastItem = this.$outerRepeater
    .find('[data-repeater-list="outer-group"] > [data-repeater-item]')
    .last();
  ...
  assert.strictEqual(
    this.$innerRepeater.find("[data-repeater-item]").length,
    1,
    "does not add item to first inner repeater"
  );

  assert.strictEqual(
    $lastItem.find("[data-repeater-item]").length,
    2,
    "adds item to second inner repeater"
  );
...
QUnit.test(
  "delete added item nested inner from repeated outer",
  function (assert) {
    this.$outerRepeater.repeater({
      repeaters: [{ selector: ".inner-repeater" }],
    });
    ...
    $lastOuterItem
      .find("[data-repeater-list] [data-repeater-delete]")
      .first()
      .click();
