    ...
    return function(e) {
        ...
        class i {
            ...
            load() {
				...
				//setTimeout(() => this.dispatch("load", "load"), 1000);
				setTimeout(() => this.dispatch("load", "load"), 200);
            }
            ...
            click(e) {
				...
				this.cur_clicked = e.target;
				//console.log('[click] e.target.classList[0] : ' + e.target.classList[0]);
				//console.log('[click] e.target.classList[1] : ' + e.target.classList[1]);
				console.log('[click] e.target.className : ' + e.target.className);
				console.log('[click] typeof this.cur_clicked.className : ' + typeof this.cur_clicked.className);

				if(typeof this.cur_clicked.className == 'string' && this.cur_clicked.className.indexOf('output') == 0){
					/* custom start */
					console.log('[click] custom click!! ' + this.node_selected);
					var l = this.ele_selected.closest(".drawflow-node").id;
					var	c = this.cur_clicked.classList[1];
					// l : output nodeDivId
					// s : input nodeDivId
					// c : output button class name
					// output_0 : 게이트웨이 out 버튼
					var outputNodeId = l.substring('node-'.length);
					var outputNodeName = this.getNodeNameCustom(outputNodeId);
					var length = this.drawflow.drawflow[this.module].data[outputNodeId].outputs[c].connections.length;
					console.log('[click] custom nodeName : ' + outputNodeName + '. output length : ' + length);
					if(length == 0){
						this.enableConnectionableNode(l, outputNodeName, c);
					...
					switch (this.ele_selected.classList[0]) {
						case "drawflow-node":
							console.log('[click] drawflow-node click!! ' + this.ele_selected.id.slice(5));
							// startNode 클릭 시 클릭 이벤트 무시
							if(this.ele_selected.id.slice(5) != 1){
						...
						case "drawflow":
							console.log('[click] parent-drawflow or drawflow click!!');
							...
							break;
						case "main-path":
							console.log('[click] main-path click!!');
							...
							break;
						case "point":
							console.log('[click] point click!!');
							this.drag_point = !0, this.ele_selected.classList.add("selected");
							break;
						case "drawflow-delete":
							console.log('[click] drawflow-delete click!!');
                ...
                "touchstart" === e.type ? (this.pos_x = e.touches[0].clientX, this.pos_x_start = e.touches[0].clientX, this.pos_y = e.touches[0].clientY, this.pos_y_start = e.touches[0].clientY) : (this.pos_x = e.clientX, this.pos_x_start = e.clientX, this.pos_y = e.clientY, this.pos_y_start = e.clientY), this.dispatch("clickEnd", e)
            }
            position(e) {
				if("fixed" === this.editor_mode){
					//console.log('[position] fixed');
				} else {
					//console.log('[position] editor mode is not fixed');
            ...
            dragEnd(e) {
                ...
                if (this.drag && (this.pos_x_start == t && this.pos_y_start == n || this.dispatch("nodeMoved", this.ele_selected.id.slice(5))), this.drag_point && (this.ele_selected.classList.remove("selected"), this.pos_x_start == t && this.pos_y_start == n || this.dispatch("rerouteMoved", this.ele_selected.parentElement.classList[2].slice(14))), this.editor_selected && (this.canvas_x = this.canvas_x + -(this.pos_x - t), this.canvas_y = this.canvas_y + -(this.pos_y - n), this.editor_selected = !1), !0 === this.connection)
                    if ("input" === i.classList[0] || this.force_first_input && (null != i.closest(".drawflow_content_node") || "drawflow-node" === i.classList[0])) {
						...
						/* custom start */
						l = this.ele_selected.closest(".drawflow-node").id;
						c = this.cur_clicked.classList[1];
						this.disableConnectionableNode();
						// l : output nodeDivId
						// s : input nodeDivId
						// c : output button class name
						// output_0 : 게이트웨이 out 버튼
						var outputNodeId = l.substring('node-'.length);
						var outputNodeName = this.getNodeNameCustom(outputNodeId);
						var inputNodeId = s.substring('node-'.length);
						var inputNodeName = this.getNodeNameCustom(inputNodeId);
						console.log('[dragEnd] nodeName >> output : ' + outputNodeName + ', input : ' + inputNodeName);
						var flag = nodeValidationAlert(outputNodeName, inputNodeName, c);
						if(!flag){
            ...
            zoom_reset() {
                ...
                0.8 != this.zoom && (this.zoom = 0.8, this.zoom_refresh())
            }
            ...
            updateConnection(e, t) {
                ...
                o = o || 0;
				console.log('[updateConnection] s : ' + s + ', o : ' + o);
                var l = this.connection_ele.children[0],
					...
					/* custom start */
                    c = this.cur_clicked.offsetWidth / 2 + (this.cur_clicked.getBoundingClientRect().x - n.getBoundingClientRect().x) * s,
                    d = this.cur_clicked.offsetHeight / 2 + (this.cur_clicked.getBoundingClientRect().y - n.getBoundingClientRect().y) * o,
                    /* custom end */
                    ...
                    if (null === f[t].querySelector(".point")) {
                        ...
                        n.forEach((t, a) => {
                            if (0 === a && n.length - 1 == 0) {
                                ...
                                m = (R = E.querySelectorAll("." + t.parentElement.classList[4])[0]).offsetWidth / 2 + (R.getBoundingClientRect().x - s.getBoundingClientRect().x) * u, _ = R.offsetHeight / 2 + (R.getBoundingClientRect().y - s.getBoundingClientRect().y) * p, w = (f.getBoundingClientRect().x - s.getBoundingClientRect().x) * u + r, v = (f.getBoundingClientRect().y - s.getBoundingClientRect().y) * p + r, y = l(w, v, m, _, d, "close");
                                o += y, g.push(y)
                            } else if (0 === a) {
                                var L;
                                f = i.querySelector("#" + e), m = ((x = t).getBoundingClientRect().x - s.getBoundingClientRect().x) * u + r, _ = (x.getBoundingClientRect().y - s.getBoundingClientRect().y) * p + r, w = (L = f.querySelectorAll("." + t.parentElement.classList[3])[0]).offsetWidth / 2 + (L.getBoundingClientRect().x - s.getBoundingClientRect().x) * u, v = L.offsetHeight / 2 + (L.getBoundingClientRect().y - s.getBoundingClientRect().y) * p, y = l(w, v, m, _, d, "open");
                                o += y, g.push(y);
                                f = t, m = ((x = n[a + 1]).getBoundingClientRect().x - s.getBoundingClientRect().x) * u + r, _ = (x.getBoundingClientRect().y - s.getBoundingClientRect().y) * p + r, w = (f.getBoundingClientRect().x - s.getBoundingClientRect().x) * u + r, v = (f.getBoundingClientRect().y - s.getBoundingClientRect().y) * p + r, y = l(w, v, m, _, c, "other");
                                o += y, g.push(y)
                            } else if (a === n.length - 1) {
                                var E, R;
                                f = t, C = t.parentElement.classList[1].replace("node_in_", ""), x = (E = i.querySelector("#" + C)).querySelectorAll("." + t.parentElement.classList[4])[0], m = (R = E.querySelectorAll("." + t.parentElement.classList[4])[0]).offsetWidth / 2 + (R.getBoundingClientRect().x - s.getBoundingClientRect().x) * u, _ = R.offsetHeight / 2 + (R.getBoundingClientRect().y - s.getBoundingClientRect().y) * p, w = (f.getBoundingClientRect().x - s.getBoundingClientRect().x) * (s.clientWidth / (s.clientWidth * h)) + r, v = (f.getBoundingClientRect().y - s.getBoundingClientRect().y) * (s.clientHeight / (s.clientHeight * h)) + r, y = l(w, v, m, _, d, "close");
                                o += y, g.push(y)
                            } else {
                                f = t, m = ((x = n[a + 1]).getBoundingClientRect().x - s.getBoundingClientRect().x) * (s.clientWidth / (s.clientWidth * h)) + r, _ = (x.getBoundingClientRect().y - s.getBoundingClientRect().y) * (s.clientHeight / (s.clientHeight * h)) + r, w = (f.getBoundingClientRect().x - s.getBoundingClientRect().x) * (s.clientWidth / (s.clientWidth * h)) + r, v = (f.getBoundingClientRect().y - s.getBoundingClientRect().y) * (s.clientHeight / (s.clientHeight * h)) + r, y = l(w, v, m, _, c, "other");
                                o += y, g.push(y)
                            }
                        }), a ? g.forEach((e, n) => {
                    ...
                    if (null === g[t].querySelector(".point")) {
                        ...
                        n.forEach((t, a) => {
                            if (0 === a && n.length - 1 == 0) {
                                ...
                                g = (x = L.querySelectorAll("." + t.parentElement.classList[3])[0]).offsetWidth / 2 + (x.getBoundingClientRect().x - s.getBoundingClientRect().x) * u, m = x.offsetHeight / 2 + (x.getBoundingClientRect().y - s.getBoundingClientRect().y) * p, _ = (f.getBoundingClientRect().x - s.getBoundingClientRect().x) * u + r, w = (f.getBoundingClientRect().y - s.getBoundingClientRect().y) * p + r, v = l(g, m, _, w, d, "open");
                                o += v, h.push(v)
                            } else if (0 === a) {
                                var x;
                                f = t, y = t.parentElement.classList[2].replace("node_out_", ""), C = (L = i.querySelector("#" + y)).querySelectorAll("." + t.parentElement.classList[3])[0], g = (x = L.querySelectorAll("." + t.parentElement.classList[3])[0]).offsetWidth / 2 + (x.getBoundingClientRect().x - s.getBoundingClientRect().x) * u, m = x.offsetHeight / 2 + (x.getBoundingClientRect().y - s.getBoundingClientRect().y) * p, _ = (f.getBoundingClientRect().x - s.getBoundingClientRect().x) * u + r, w = (f.getBoundingClientRect().y - s.getBoundingClientRect().y) * p + r, v = l(g, m, _, w, d, "open");
                                o += v, h.push(v);
                                f = t, _ = ((C = n[a + 1]).getBoundingClientRect().x - s.getBoundingClientRect().x) * u + r, w = (C.getBoundingClientRect().y - s.getBoundingClientRect().y) * p + r, g = (f.getBoundingClientRect().x - s.getBoundingClientRect().x) * u + r, m = (f.getBoundingClientRect().y - s.getBoundingClientRect().y) * p + r, v = l(g, m, _, w, c, "other");
                                o += v, h.push(v)
                            } else if (a === n.length - 1) {
                                var L, E;
                                f = t, y = t.parentElement.classList[1].replace("node_in_", ""), C = (L = i.querySelector("#" + y)).querySelectorAll("." + t.parentElement.classList[4])[0], _ = (E = L.querySelectorAll("." + t.parentElement.classList[4])[0]).offsetWidth / 2 + (E.getBoundingClientRect().x - s.getBoundingClientRect().x) * u, w = E.offsetHeight / 2 + (E.getBoundingClientRect().y - s.getBoundingClientRect().y) * p, g = (f.getBoundingClientRect().x - s.getBoundingClientRect().x) * u + r, m = (f.getBoundingClientRect().y - s.getBoundingClientRect().y) * p + r, v = l(g, m, _, w, d, "close");
                                o += v, h.push(v)
                            } else {
                                f = t, _ = ((C = n[a + 1]).getBoundingClientRect().x - s.getBoundingClientRect().x) * u + r, w = (C.getBoundingClientRect().y - s.getBoundingClientRect().y) * p + r, g = (f.getBoundingClientRect().x - s.getBoundingClientRect().x) * u + r, m = (f.getBoundingClientRect().y - s.getBoundingClientRect().y) * p + r, v = l(g, m, _, w, c, "other");
                                o += v, h.push(v)
            ...
            createReroutePoint(e) {
                ...
                void 0 === this.drawflow.drawflow[this.module].data[r].outputs[i].connections[h].points && (this.drawflow.drawflow[this.module].data[r].outputs[i].connections[h].points = []), this.reroute_fix_curvature ? (d > 0 ? this.drawflow.drawflow[this.module].data[r].outputs[i].connections[h].points.splice(d, 0, {
                    pos_x: l,
                    pos_y: c
			...
			getNodeNameCustom(e){
				...
				var name = this.drawflow.drawflow[t].data[e].name;
				return name;
			}
            getNodeFromId(e) {
                var t = this.getModuleFromNodeId(e);
			...
			enableConnectionableNode(nodeDivId, outputNodeName, outputClassName){
				const nodeClass = 'drawflow-node';
				var nodeId = '';
				var nodeName = '';
				var flag = false;
				this.container.querySelectorAll("." + nodeClass).forEach((e, t) => {
					// t : index
					flag = false;
					nodeId = e.id.substring('node-'.length);
					nodeName = this.getNodeNameCustom(nodeId);

					if(nodeName != 'startNode'){
						flag = this.nodeValidation(outputNodeName, nodeName, outputClassName);
						if(flag){
							if(e.id != nodeDivId){
								e.classList.add("connectionabled");
							}
						}
					}
                })
			}
			disableConnectionableNode(){
				const nodeClass = 'drawflow-node';
				this.container.querySelectorAll("." + nodeClass).forEach((e, t) => {
                    e.classList.remove("connectionabled");
                })
			}
			nodeValidation(outputNodeName, inputNodeName, outputClassName){
				...
				if(outputNodeName == 'gatewayNode') {
					if(inputNodeName == 'gatewayNode' || inputNodeName == 'metaNode' || inputNodeName == 'slotNode'){
						return false;
					}
				}

				if(outputNodeName == 'apiNode') {
					if(inputNodeName == 'metaNode' || outputNodeName == 'apiNode'){
						return false;
					}
            ...
            addNode(e, t, n, i, s, o, l, c, d = !1) {
                ...
                for (var g = 0; g < t; g++) {
                    ...
                    e.classList.add("input"), e.classList.add("input_" + (g + 1)), f["input_" + (g + 1)] = {
                        connections: []
                    }, u.appendChild(e)
                }
                const m = {};
                for (g = 0; g < n; g++) {
                    const e = document.createElement("div");
                    e.classList.add("output"), e.classList.add("output_" + (g + 1)), m["output_" + (g + 1)] = {
                        connections: []
                    }, p.appendChild(e)
                }
            ...
            getNodeOutputData(e){
            	return this.drawflow.drawflow[this.module].data[e].data;
            }
			updateHtml(e){
				// e : nodeId 
				var html = this.container.querySelector("#node-" + e + " .drawflow_content_node").innerHTML;
				this.drawflow.drawflow[this.module].data[e].html = html;
			}
			updateNodeOutputDetail(e, idx, btnLabel, detailDataObj){
				if(typeof this.drawflow.drawflow[this.module].data[e].data['name'] == "undefined"){
					var nameObj = new Object();
					this.drawflow.drawflow[this.module].data[e].data['name'] = nameObj;
				}
				this.drawflow.drawflow[this.module].data[e].data['name'][idx] = btnLabel;
				
				if(typeof this.drawflow.drawflow[this.module].data[e].data['detail'] == "undefined"){
					var detailObj = new Object();
					this.drawflow.drawflow[this.module].data[e].data['detail'] = detailObj;
				}
				this.drawflow.drawflow[this.module].data[e].data['detail'][idx] = detailDataObj;
				console.log(this.drawflow.drawflow['Home'].data[e].data);
			}
			updateNodeOutputSort(e, idx, sortIdx){
				if(typeof this.drawflow.drawflow[this.module].data[e].data['sort'] == "undefined"){
					var sortObj = new Object();
					this.drawflow.drawflow[this.module].data[e].data['sort'] = sortObj;
				}
				this.drawflow.drawflow[this.module].data[e].data['sort'][idx] = sortIdx;
				console.log(this.drawflow.drawflow['Home'].data[e].data);
			}
			updateNodeApiData(e, apiNo, paramArr){
				this.drawflow.drawflow[this.module].data[e].data['api'] = apiNo;
				this.drawflow.drawflow[this.module].data[e].data['param'] = paramArr;
				console.log(this.drawflow.drawflow['Home'].data[e].data);
			}
			updateNodeSlotData(e, slotArr){
				this.drawflow.drawflow[this.module].data[e].data['slot'] = slotArr;
				console.log(this.drawflow.drawflow['Home'].data[e].data);
			}
			updateNodeMetaData(e, type, refId){
				if(typeof this.drawflow.drawflow[this.module].data[e].data['meta'] == "undefined"){
					var metaObj = new Object();
					this.drawflow.drawflow[this.module].data[e].data['meta'] = metaObj;
				}
				if(typeof type != "undefined" && type != null && type != "" &&
						typeof refId != "undefined" && refId != null && refId != ""){
					this.drawflow.drawflow[this.module].data[e].data['meta'].type = type;
					this.drawflow.drawflow[this.module].data[e].data['meta'].value = refId;
				}
				console.log(this.drawflow.drawflow['Home'].data[e].data);
			}
			updateNodeImageValueCustom(e, imgId){
				// e : nodeId
				if(imgId == -1){
					this.drawflow.drawflow[this.module].data[e].data['image'] = null;
				} else {
					this.drawflow.drawflow[this.module].data[e].data['image'] = imgId;					
				}
			}
            addNodeImport(e, t) {
				/*
				console.log('[addNodeImport] t value start');
				console.log(t);
				console.log('[addNodeImport] t value end');
				*/
            ...
            addNodeOutput(e, outIdx) {
                var t = this.getModuleFromNodeId(e);
				const n = this.getNodeFromId(e),
                    i = Object.keys(n.outputs).length;
				console.log('[addNodeOutput] btnOutIdx : ' + outIdx);
				/* custom start */
				if (this.module === t) {
                    const t = document.createElement("div");
					t.classList.add("output"), t.classList.add("output_" + outIdx), this.container.querySelector("#node-" + e + " .button-" + outIdx).appendChild(t), this.updateConnectionNodes("node-" + e)					
                }
                this.drawflow.drawflow[t].data[e].outputs["output_" + outIdx] = {
                    connections: []
                }
				/* custom end */
            ...
            removeNodeOutputCustom(e, t) {
                var n = this.getModuleFromNodeId(e);
                const i = this.getNodeFromId(e);
				console.log("[removeNodeOutputCustom] #node-" + e + " .output." + t);
				this.module === n && this.container.querySelector("#node-" + e + " .output." + t).remove();
                const s = [];
                Object.keys(i.outputs[t].connections).map((function(n, o) {
                    const l = i.outputs[t].connections[o].node,
                        c = i.outputs[t].connections[o].output;
            ...
            removeNodeOutputDataCustom(e, idx){
            	if(typeof this.drawflow.drawflow[this.module].data[e].data['name'] != "undefined" && typeof this.drawflow.drawflow[this.module].data[e].data['name'][idx] != "undefined"){
            		delete this.drawflow.drawflow[this.module].data[e].data['name'][idx];
            		console.log('[removeNodeOutputDataCustom] remove name id : ' + idx);
            	}
            	if(typeof this.drawflow.drawflow[this.module].data[e].data['detail'] != "undefined" && typeof this.drawflow.drawflow[this.module].data[e].data['detail'][idx] != "undefined"){
            		delete this.drawflow.drawflow[this.module].data[e].data['detail'][idx];
            		console.log('[removeNodeOutputDataCustom] remove detail id : ' + idx);
            	}
            	if(typeof this.drawflow.drawflow[this.module].data[e].data['sort'] != "undefined" && typeof this.drawflow.drawflow[this.module].data[e].data['sort'][idx] != "undefined"){
            		delete this.drawflow.drawflow[this.module].data[e].data['sort'][idx];
            		console.log('[removeNodeOutputDataCustom] remove sort id : ' + idx);
            	}
            }
            removeNodeId(e) {
            ...
            changeModule(e) {
                this.dispatch("moduleChanged", e), this.module = e, this.precanvas.innerHTML = "", this.canvas_x = 0, this.canvas_y = 0, this.pos_x = 0, this.pos_y = 0, this.mouse_x = 0, this.mouse_y = 0, this.zoom = 0.8, this.zoom_last_value = 0.8, this.precanvas.style.transform = "", this.import(this.drawflow, !1)
            }
            removeModule(e) {
