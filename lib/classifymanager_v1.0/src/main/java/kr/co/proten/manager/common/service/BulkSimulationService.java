...
public abstract class BulkSimulationService {
	 ...
	 * @throws Exception
	 */
	protected void doFileValidation(String[] HEADER_NAME, File file, BulkSimulationResultRootVo resultVo) throws Exception {
		...
		if(lastHeaderCellNum != HEADER_NAME.length) {
			...
			resultVo.setErrorMessage("업로드 파일 양식이 잘못되었습니다.");
			if(workbook != null) {
				workbook.close();
			}
			if(fis != null) {
		...
		for(int idx=0; idx<HEADER_NAME.length; idx++) {
			...
			if(headerCell.getRichStringCellValue() == null) {
				resultVo.setStatus("fail");
				resultVo.setErrorMessage("[" + HEADER_NAME[idx] + "] 헤더가 일치 하지 않습니다.");
				if(workbook != null) {
					workbook.close();
				}
				if(fis != null) {
	 ...
	 * @return
	 */
	public void doImportData(BulkSimulationResultRootVo resultVo) throws Exception {
	};
