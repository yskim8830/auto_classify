...
class question():
    ...
    def word2vec_question(self, question, question_title):
        ...
        try:
            ...
            total_weight = 0
            mean_query = ''
            mean_idx = {'index' : model_idx}
            for key in r_question:
                mean_query += '{}\n{}\n'.format(json.dumps(mean_idx), json.dumps(es.question_query_to_vector(self.version,key)))
            mean_result = es.msearch(mean_query,'vector')
            
            for idx, result in enumerate(mean_result):
                if result != -1:
